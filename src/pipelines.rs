...
#[must_use]
pub fn create_2d_pipeline(name: &str, vertex_buffer_layouts: &[Option<wgpu::VertexBufferLayout>], vertex_shader: &wgpu::ShaderModule, fragment_shader: &wgpu::ShaderModule, output_format: &wgpu::TextureFormat, gpu_instance: &GpuInstance) -> wgpu::RenderPipeline {
	gpu_instance.wgpu_device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
		label: Some(name),
...
#[must_use]
pub fn create_3d_pipeline(name: &str, vertex_buffer_layouts: &[Option<wgpu::VertexBufferLayout>], vertex_shader: &wgpu::ShaderModule, fragment_shader: &wgpu::ShaderModule, output_format: &wgpu::TextureFormat, gpu_instance: &GpuInstance) -> wgpu::RenderPipeline {
	gpu_instance.wgpu_device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
		label: Some(name),
		layout: Some(&gpu_instance.wgpu_pipeline_layout),
		vertex: wgpu::VertexState {
...
pub fn start_2d_render_pass<'a>(name: &str, texture_output: &wgpu::TextureView, clear_color: Option<wgpu::Color>, command_encoder: &'a mut wgpu::CommandEncoder) -> wgpu::RenderPass<'a> {
...
pub fn start_3d_render_pass<'a>(name: &str, texture_output: &wgpu::TextureView, depth_tex: &wgpu::TextureView, clear_color: Option<wgpu::Color>, clear_depth: bool, command_encoder: &'a mut wgpu::CommandEncoder) -> wgpu::RenderPass<'a> {
	let load_od = if let Some(clear_color) = clear_color {
		wgpu::LoadOp::Clear(clear_color)
	} else {
		wgpu::LoadOp::Load
...
pub fn render(render_pass: &mut wgpu::RenderPass, pipeline: &wgpu::RenderPipeline, vertex_buffers: &[&wgpu::Buffer], index_buffer: Option<&IndexBuffer>, texture_input: &Texture, uniforms: &wgpu::BindGroup, vertex_count: u32, instance_count: u32) {
	...
	if let Some(index_buffer) = index_buffer {
		render_pass.set_index_buffer(index_buffer.wgpu_buffer.slice(..), wgpu::IndexFormat::Uint16);

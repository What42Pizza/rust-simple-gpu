...
pub struct Texture {
	...
	/// Can be given to bind groups, and specifies how the texture is sampled
...
/// Creates a new texture with a given size and format
#[inline]
#[must_use]
pub fn create_texture(name: &str, size: (u32, u32), format: wgpu::TextureFormat, sampler: &wgpu::Sampler, gpu_instance: &GpuInstance) -> Texture {
	
	let texture = gpu_instance.wgpu_device.create_texture(&wgpu::TextureDescriptor {
		label: Some(name),
		size: wgpu::Extent3d {
			width: size.0,
			height: size.1,
	...
	let bind_group = gpu_instance.wgpu_device.create_bind_group(&wgpu::BindGroupDescriptor {
		label: Some(name),
...
pub fn create_depth_texture(name: &str, size: (u32, u32), gpu_instance: &GpuInstance) -> Texture {
	...
	let sampler = make_sampler(wgpu::AddressMode::ClampToEdge, wgpu::FilterMode::Linear, gpu_instance);
...
/// Creates a texture from a given file path. The result always uses the format `wgpu::TextureFormat::Rgba8Unorm`
/// 
/// # Errors
/// 
/// This only errors if it fails to read the file
#[cfg(feature = "image")]
pub fn load_texture_from_path(path: &Path, sampler: &wgpu::Sampler, gpu_instance: &GpuInstance) -> Result<Texture> {
	let texture_image = image::open(path).with_context(|| format!("Failed to read file {}", path.display()))?;
	let size = (texture_image.width(), texture_image.height());
	let texture_data = texture_image.into_rgba8();
	let file_name = file_name(path).ok_or_else(|| anyhow!("Failed to get file name from path {}", path.display()))?;
	let texture = create_texture(&file_name, size, wgpu::TextureFormat::Rgba8Unorm, sampler, gpu_instance);
...
#[must_use]
pub fn make_sampler(wrapping: wgpu::AddressMode, filter: wgpu::FilterMode,  gpu_instance: &GpuInstance) -> wgpu::Sampler {
	gpu_instance.wgpu_device.create_sampler(&wgpu::SamplerDescriptor {
